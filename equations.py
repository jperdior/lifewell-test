...
class Equations:
    @staticmethod
    def update_average(average_n_minus_1: float, n: int, x_n: int) -> float:
        return average_n_minus_1 + (x_n - average_n_minus_1) / n

    @staticmethod
    def update_average_with_delete(average_n: float, n: int, x_n: int) -> float:
    ...
    @staticmethod
    def calculate_variance_from_delete(
        std_deviation_n: float,
        x_n: float,
        average_n: float,
        average_n_minus_1: float,
        n: int,
        ...
        return ((variance_n * n) - (x_n - average_n) * (x_n - average_n_minus_1)) / (
            n - 1
        )
